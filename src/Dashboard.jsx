...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#0A1C2F] text-white p-6 relative flex flex-col justify-between">
      <div>
        ...
        {showSection === 'subscribe' && (
          <div className="max-w-xl mx-auto">
            ...
            <input list="token-list" className="w-full p-2 mb-2 rounded text-black" placeholder="Enter Token Name" value={tokenAddress} onChange={(e) => setTokenAddress(e.target.value)} />
            <datalist id="token-list">
              {tokenOptions.map((token, index) => (
                <option key={index} value={token} />
              ))}
            </datalist>
            <input className="w-full p-2 mb-2 rounded text-black" placeholder="Enter Email Address" value={email} onChange={(e) => setEmail(e.target.value)} />
            ...
            <input className="w-full p-2 mb-2 rounded text-black" placeholder="Remind me X days before unlock" value={reminderDays} onChange={(e) => setReminderDays(e.target.value)} />
            <button onClick={checkEligibility} className="bg-green-600 px-4 py-2 rounded mb-2">Check Eligibility</button>
            {eligibility && <p className="mb-2 text-yellow-400">{eligibility}</p>}
            <div className="mb-2">
              <label>
      ...
      {!showSection && (
        <div className="text-center mt-12">
          <h1 className="text-3xl font-extrabold text-green-300 mb-2">JOIN TO LAUNCH!</h1>
          <p className="text-2xl font-bold text-green-400">$JARVIS is live on <br /> Virtual Protocol</p>
          <a href="https://app.virtuals.io/geneses/3538" target="_blank" rel="noopener noreferrer">
            <button className="mt-4 bg-blue-700 text-white px-6 py-2 rounded-full font-bold">JOIN</button>
          </a>
        </div>
      )}
    </div>
